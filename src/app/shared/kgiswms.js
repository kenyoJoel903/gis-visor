L.TileLayer.KGisWMS = L.TileLayer.WMS.extend({
	...
	showGetFeatureInfo : function(err, latlng, content) {	
		...
		if ( content.features.length > 1 ){
			...
			popupCarrousel.setAttribute('style', 'margin: 0px -20px; margin-right: -26px; margin-top: 15px;');
		...
		for (var i = 0; i < content.features.length; i++) {			
			...
			let infoValues = {};
			let geoJson = from.geometry;
			if (this.infoAlias) {
				for ( let name in this.infoAlias) {
					infoValues[this.infoAlias[name].aliasInfo] = from.properties[name+'_Alias'] || from.properties[name] || '';
					if (infoValues[this.infoAlias[name].aliasInfo] == "" || infoValues[this.infoAlias[name].aliasInfo] == null) {
						infoValues[this.infoAlias[name].aliasInfo] = "-";
					}
					if (infoValues[this.infoAlias[name].aliasInfo] != null && this.infoAlias[name].type=='DATE') {
			...
			try {
				...
				this.onCreatePopupContent(infoValues,popup_content,div_botones, pkLayer, geoJson);
				isshow = true;
			} catch (e) {
				isshow = false;
				console.log(e)
...
export var kgisWMS = function(url, options) {
	return new L.TileLayer.KGisWMS(url, options);
};